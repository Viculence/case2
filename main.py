...
def main():
        ...
        try:
            ...
            for key in all_norm_result:
                for sub_key in all_norm_result[key]:
                    (all_norm_result[key][sub_key].extend
                     (norm_result[key][sub_key]))

            # 4. Secrets (find_secrets возвращает список строк)
            secrets = find_secrets(text)
            ...
            print(f"[{len(cards_result['valid'])}] ВАЛИДНЫЕ КАРТЫ (Luhn OK): "
                  f"{cards_result['valid'][:3]}")
            print(f"[{len(secrets)}] Секретов найдено: {secrets[:3]}")
            print(f"[{len(system_info['ips'])}] IP-адреса: "
                  f"{system_info['ips'][:3]}")
            print(f"[{len(decoded['base64'])}] Base64 сообщений найдено")
            print(f"[{len(logs_result['sql_injections'])}] "
                  f"SQL-инъекций найдено")
    ...
    for key in all_norm_result:
        for sub_key in all_norm_result[key]:
            all_norm_result[key][sub_key] = list(set(all_norm_result
                                                     [key][sub_key]))
    all_secrets = list(set(all_secrets))
    for key in all_system_info:
        all_system_info[key] = list(set(all_system_info[key]))
    for key in all_decoded:
        all_decoded[key] = list(set(all_decoded[key]))
    ...
    print(f"\n[{sum(len(v) for v in all_logs_result.values())}] "
    ...
    print(f"\n[{sum(len(v) for v in all_decoded.values())}] "
          f"РАСШИФРОВАННЫЕ СООБЩЕНИЯ:")
    for cat, items in all_decoded.items():
        if items:
            print(f"  {cat}: {len(items)} найдено")

    # СБОРКА АРТЕФАКТОВ (невалидные данные)
    ...
    for items in all_logs_result.values():
        all_unique_artifacts.update(items)

    print(f"\n------------------------------------------------")
    print(f"Количество уникальных артефактов: {len(all_unique_artifacts)}")
    ...
    try:
        with open(artifacts_file, "w", encoding="utf-8") as f_out:
            ...
            f_out.write(f"Generated: {datetime.datetime.now().strftime
            ('%Y-%m-%d %H:%M:%S')}\n\n")
